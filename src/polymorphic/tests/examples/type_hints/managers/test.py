...
class TypeHintsManagersTest(TestCase):
    def test_type_hints(self):
        ...
        _1: t.Optional[ParentModel | Child1 | Child2] = ParentModel.objects.first()
        assert _1 == parent or _1 == child1 or _1 == child2
        assert _1 == parent
        _2: t.Optional[ParentModel | Child1 | Child2] = ParentModel.objects.order_by("pk").first()
        assert _2 == parent
        ...
        _7: t.Optional[ParentModel] = ParentModel.objects.non_polymorphic().first()  # type: ignore[assignment]
        assert _7 == parent
        ...
        assert _11 == child2
        _12: Child2 = Child2.objects.filter().non_polymorphic().get(pk=child2.pk)
        assert _12 == child2

        _13: t.Optional[ParentModel] = ParentModel.objects.instance_of(ParentModel).first()
        assert _13 == parent
        _14: t.Optional[ParentModel] = ParentModel.objects.all().instance_of(ParentModel).first()
        assert _14 == parent

        _15: t.Optional[Child1] = ParentModel.objects.instance_of(Child1).first()
        assert _15 == child1
        _16: t.Optional[Child1] = ParentModel.objects.all().instance_of(Child1).first()
        assert _16 == child1
        _17: t.Optional[Child2] = ParentModel.objects.instance_of(Child2).first()
        assert _17 == child2
        _18: t.Optional[Child2] = ParentModel.objects.all().instance_of(Child2).first()
        assert _18 == child2
